...
setwd("/Users/aciernia/Sync/CierniaLab/Members/AnnieCiernia/Experiments/MIA_placenta_brainRNAseq/Analysis/DEGs")
...
countmatrix <- out.file[,2:ncol(out.file)]
...
#set design matrix
design <- model.matrix(~0+group+sex+uterine.horn.position, data=info) #if using a 0 intercept must set up contrasts
...
info$MAA <- paste(info$group,info$SampleID,sep=".")
...
countdf3 <-merge(countdf2,info,by.x="sample",by.y="SampleID")
...
write.csv(GeneSummary2,file= "Log2CPM_GeneExpression.csv")
...
countdf3$Individual <- factor(countdf3$MAA)
...
save(GeneSummary2,GeneSummaryCPM2,IndGeneSummary2,file="AverageCPMandLog2CPM.RData")
...
#no log fc filter:
write.xlsx(sum,"SummaryCount_DEGs.xlsx")
...
write.xlsx(SumTableOut,"SummaryTableDEgenes.xlsx")
